...
def main(args):
    ...
    for speaker in tqdm(
        glob.glob(os.path.join(split_path, "p*")), ascii=True, desc="Speaker"
    ):
        speaker_count += 1
        output_folder = os.path.join(
            args.output_path, args.split, speaker.split("/")[-1]
        )
        ...
        with torch.no_grad():
            for audio_path in glob.glob(os.path.join(split_path, speaker, "*.wav")):
                audio_count += 1
